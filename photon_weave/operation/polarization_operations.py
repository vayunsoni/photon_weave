        ...
        match  operation:
            case PolarizationOperationType.RX:
                if "theta" not in self.kwargs:
                    raise KeyError(
                        "The argument 'theta' is required for RX gate"
                    )
            case PolarizationOperationType.RY:
                if "theta" not in self.kwargs:
                    raise KeyError(
                        "The argument 'theta' is required for RY gate"
                    )
            case PolarizationOperationType.RZ:
                if "theta" not in self.kwargs:
                    raise KeyError(
                        "The argument 'theta' is required for RZ gate"
                    )
            case PolarizationOperationType.U3:
                if ("theta" not in self.kwargs or
                    "lambda" not in self.kwargs or
                    "phi" not in self.kwargs):
                    raise KeyError(
